...
| 49 | **WP8 done — decision KEEP; and `disputed`+evidence was silently closing the gate** | **The full 51.8KB evaluation, plus the third instance of the protocol flattering itself.** Eval in `docs/eval/full-20260908.md`. **The defect first:** WP8 attempt 1 finished at R2 reporting `openHigh: 0` while five evidenced high-severity findings sat unresolved. `isUnsettled` treated `disputed` **with** evidence as settled; §13.34 had closed only the unevidenced complement, so the remaining move was to dispute your own evidenced finding and it left the gate. `disputed` means the parties disagree, which is unresolved by definition — §8.4 says so itself ("the minority report is never empty when any claim is `disputed`"). Now only `resolved`+evidence or `withdrawn` settles a claim. Measured on the identical seed: rounds 2→3, claims 19→27, reported open high **0 (false) → 8**. Without the fix WP8 would have recorded a *better-looking* result — fewer open items, faster — that was wrong. That is now three defects of the same species (§13.34, §13.35, §13.49): **the protocol's characteristic failure is making its output look cleaner than reality**, which is far more dangerous than crashing. Two existing tests asserted the old behaviour and had to be inverted; their comments claimed parking an evidenced claim was "legitimate", which WP8 disproved. **The result:** on `tier: "ibm"` both arms cost **$0.00**, so WP5's 16.6× objection — the strongest argument against the whole idea — simply disappears. Debate: 671K tokens, 22 requests, 10m27s, **36 bash calls**, 3 rounds, 27 claims **27/27 evidenced**, 8 high+ all evidenced. Baseline: 339K tokens, 9 requests, 5m54s, 21 bash calls, 7 verified flaws + self-critique. **7 of 10 topics were found independently by BOTH arms** — real corroboration at scale, versus one overlap at 6KB. Debate-only: 4 findings including a `critical` (trigger-less autopilots outside `factory stop`'s scoped iteration, proven via `--all-workspaces`), and B8/B9 which **critique the Ideator's proposed remedies** — findings that cannot exist without an adversarial pass, since a single reviewer has no proposal to attack. **Baseline-only: 3 findings the debate missed**, two of them simple machine facts (23 GiB free vs the plan's 95 GiB; 115 GiB omitted from the "independent copy"). That is a real gap, reported not buried. **Decision: KEEP** as an explicit `/debate`, defaulting to `tier: "ibm"` so invoking it is free — **and run the baseline too when the decision is irreversible**, because the union of both arms (10 topics) beats either alone. Not automatic, not a replacement for a strong single-model review. Caveats recorded in the eval: the baseline needed one retry after a transient provider blip (§13.38's pattern, 17m41s of timeouts); the comparison is single-shot per arm; and the topic-overlap table is my own keyword analysis, not blind grading, by the same agent that built the system. **§4.1's cost table should now be rewritten** against these numbers — its $1–2/turn projection was wrong by an order of magnitude in both directions depending on provider. |
| 50 | **`timeouts.verdictGraceMs` added — the judge turn was the one unbounded step** | **User question after WP8: can we bound operation time, not just budget, especially on free tiers? Investigated rather than assumed.** A total wall-clock cap already existed (`timeouts.totalMs`, default 900s) and is checked at three points — before each round, mid-round before each sequential turn, and before a repair — and it degrades correctly: verified with an injected clock that an exhausted budget yields `status: partial`, zero model turns, and **a verdict file still written**, never a hang and never nothing. **But there was a real hole:** `budgetStop()` breaks the round loop and then `runVerdict()` runs the judge with no total-time check at all. Reproduced with a fake clock: budget exhausted after R1, rounds stopped — and the judge ran anyway, unbounded. That is the realistic runaway on a zero-dollar tier, because the judge is the single largest turn (the whole seed can be inlined for it, `inlineFullSeedUnderChars`) and IBM is measurably slower per turn than OpenRouter. **Fix:** `timeouts.verdictGraceMs` (default 300s), the extra wall clock the judge may use after `totalMs` is gone. It bounds both *whether* the judge starts and *how long* its turn may run (`timeoutMs` is clamped to the remaining grace, so a large `turnMs` cannot outrun it). Past the hard stop the judge is skipped and `verdict.ts`'s mechanical stand-in — unresolved items listed, decision explicitly "no judge turn completed" — is written instead. **The default is deliberately non-zero:** `totalMs` stops rounds but must NOT kill the verdict, because a run that spends its budget and returns nothing is worse than one that returns a `partial` conclusion; `verdictGraceMs: 0` forbids it for anyone who disagrees. `validate()` backfills the key so pre-existing configs keep working and a negative value is normalized rather than silently disabling the judge. 14 new checks covering all four paths (exhausted before R1 / mid-run with the judge still reachable / grace also gone / turn clamped); verified discriminating by removing the guard, which fails with "judge did NOT run once grace was 0". |
| 51 | **Usability pass: the dryRun estimate was fiction and printed `null` models** | **Found by actually using the extension as a user would, rather than asserting it worked.** Drove `/debate` and `debate_run` through a real `pi` session on a 211-char seed. It functioned end to end — but the dry run reported **"estimated cost: $3.50-$14.00"** for two lines of text, which would scare a user off a run that WP8 measured at **$0.00**. Three bugs in one block: (1) the estimate was §4.1's discredited `turns × $0.50-2` guess, wrong by ~20× on a small seed and unrelated to any measurement; (2) it ignored the resolved provider entirely, so a zero-dollar `tier: "ibm"` roster was still quoted in dollars that cannot be charged and cannot be capped (§13.14/§13.19); (3) it printed `config.models.*`, which `applyTier` **nulls out** when a tier is active — so every tiered roster displayed `null` for all three models. Fixed by extracting `estimateRun()` into `command.ts` and anchoring it on measured runs (WP5 re-probe: 6.1KB/3 rounds = $2.11; WP8: 51.8KB on ibm = $0.00), scaling by seed size and round count with a floor so a one-line idea still accounts for 7 turns of overhead. A roster where every role is on a no-price-table provider now reports **$0.00 and says why**, instead of inventing a figure. The plan output also now shows the resolved `ref` per role and the **time caps** (§13.50), which a user needs to see before committing to a run. **Why it survived this long:** `dryRun` had *zero* tests, because the logic lived in `index.ts`, which tests cannot import (§13.21) — the same structural trap as §13.44's sweep. Moving it to `command.ts` made 13 checks possible, including the specific regression ("a 211-char seed is under $1, not $14") and the mixed-roster case where one paid role must still produce a real figure. Verified live afterwards: the same seed now reports `$0.16-$0.63` with correct model refs and `time cap: 900s total, 240s per turn, +300s verdict grace`. **Lesson, again:** anything not reachable by a test will eventually be wrong, and `index.ts` is where that keeps happening. |
| 52 | **Global config written (it never was); codex quota recovered, so §13.41's provider note was stale** | **Two corrections prompted by the user asking whether I had actually written the default config. I had not — I had only told them what to write, which is not the same thing.** `~/.pi/agent/settings.json` had **no `debate` key**, so every run was on code defaults: the **paid** `openrouter` roster, drawing from the ~$13 remaining balance (§13.42) rather than running free. Written now (after backing the file up): `tier: "ibm"`, `rounds.max 3`, and time/token caps sized for the slower zero-dollar provider — `totalMs 2700s`, `turnMs 420s`, `verdictGraceMs 420s`, `budget.tokens 4M`, `perTurnTokens 700K` — because on this tier USD caps are inert and time/tokens are the only real limits (§13.19/§13.50). `publish` and `participate` left **off**, since both change behaviour and should be opt-in. Verified from a clean directory with no local `.pi/debate.json`: `sources: defaults -> settings.json#debate`, all three roles resolve to `ibm-services-essentials`, and a live `dryRun` reports `estimated cost: $0.00 - every role is on a provider that bills nothing` with `time cap: 2700s total, 420s per turn, +420s verdict grace`. **Second correction: the codex subscription recovered.** §13.41 recorded it exhausted on 2026-09-07; on 2026-09-08 both `gpt-5.5` and `gpt-6-astra` return `stopReason: "stop"` with real cost. So `PROVIDER_WARNINGS`' flat "usage limit was reached" claim and CONFIG.md's "currently exhausted" were both **actively wrong** — they would have steered a reader away from a working roster. Reworded to describe the *transient* nature of a quota-metered subscription rather than a one-off event, and kept rather than deleted because the failure mode (`stopReason:"error"`, "usage limit has been reached") is worth naming. The `default` roster stays on `gpt-5.6-sol`: WP8 justified it on measured cost, not on the outage. **Lesson:** environment facts recorded in §13 decay, and a stale "this is broken" note is worse than none — it is trusted and wrong. |
| 53 | **A monologue was reported as a complete debate — found by watching a real run** | **The user asked for a live run to be watched for issues. It surfaced the worst-presented failure yet, plus two tuning faults.** Run `20260908-120000-live` on the 51.8KB plan reported `status: complete`, `Rounds: 2`, `Unsettled at high+ severity: 0`, and `Confidence: 0.85`. **Every one of the four Skeptic attempts failed** — two `failed` (provider `Connection error`) in R1, two `timeout` in R2 — so all 11 claims were the Ideator's own, all `medium`, because the Ideator may not set `high` (§8.1). The reassuring `openHigh: 0` was therefore an artifact of nothing being verified, and the only signal was a mild note: "continued with one debater". A reader would reasonably have trusted that verdict. **This is the fourth defect of the §13.34/§13.35/§13.49 species and the most dangerous instance: the output looked cleanest exactly when the protocol had failed hardest.** Fixes: (1) `skepticContributed()` — a run with no merged Skeptic turn can no longer be `complete`, it is forced to `partial`; (2) a blunt note naming the attempt count and stating that the severity count "means nothing"; (3) a `skeptic_absent` event; (4) `verdict.ts` prints **NO ADVERSARIAL REVIEW HAPPENED** in the verdict *header*, derived from the absence of any author-`B` claim, since notes are easy to skim past. A control test asserts a healthy run carries none of this, so it cannot become noise. **Tuning faults, both measured not guessed:** the R2 Skeptic turn hit its 420s cap at exactly **420.1s** after 22 tool calls and 311K tokens — real verification work, then discarded as a timeout (360K tokens across all four attempts were paid for and thrown away, per §13.23). And that single turn used 311K against the `ibm` tier's 350K per-turn cap: **12% headroom**. The workspace config now sets `turnMs 900s` and a Skeptic `perTurnTokens 700K`. **Not the cause, despite appearances:** the `Model "gpt-5.6-sol" not found for provider` warning in every stderr tail is the known-harmless §13.16 false alarm — the id is present in `ibm-services-essentials-models.json:293` and three isolated probes succeeded 3/3. The failures were load-dependent provider instability on a 52KB seed with heavy tool use, which is precisely why §13.38 made `stderrTail` persistent; without it this would have been undiagnosable. |
| 54 | **`npm test` was not hermetic — writing my own global config broke two suites** | **Caught immediately after §13.52 wrote a `debate` block into `~/.pi/agent/settings.json`: `wp1` and `roles` started failing with "got ibm-services-essentials/claude-opus-4-8, want openrouter/anthropic/claude-opus-4-8".** `loadConfig()` hardcoded `join(homedir(), ".pi", "agent", "settings.json")`, so every test that exercised config resolution silently read **the developer's real global config**. The suite had been passing only because no `debate` block existed yet — it was environment-dependent all along and nobody could have known. Two consequences worth separating: the tests were wrong (not hermetic), and the *code* was also less correct than it looked, because pi itself honours `PI_AGENT_DIR` and `config.ts` ignored it. Fixed both at once: `loadConfig` now prefers `process.env.PI_AGENT_DIR` when set, and `test/run-all.sh` points it at a fresh `mktemp -d` with a `trap` cleanup, so the offline suite runs against an empty agent dir by construction. Verified in both directions: with the real global config present the suite is 8/8 green, and a direct probe shows an empty agent dir resolving `tier: null` with 1 source while the real one resolves `tier: ibm` with 2. **Lesson:** a test suite that reads any developer's home directory is a latent failure that only appears once someone configures the tool normally — which is exactly what happened here, at the moment the project became usable. |
| 55 | **Raw milliseconds made the most important safety limits hard to audit; configuring a debate had no safe guided path** | **User-requested UX correction.** `turnMs: 900000` is easy to misread and had already contributed to the 420-second cap that discarded 360K Skeptic tokens in §13.53. Config now accepts canonical readable `turn`, `total`, and `verdictGrace` values (`"90s"`, `"15m"`, `"1h"`, `"1.5h"`, `"1h30m"`), including role `budget.turn`; old `*Ms` spelling and bare numeric milliseconds remain compatible. Every value normalizes to integers before validation; malformed, negative, zero-turn, and zero-total values are **hard errors**, never an unnoticed fallback. `verdictGrace: 0` remains the intentional exception. `/debate setup` adds the missing safe route: it explicitly chooses global versus exact-cwd local config, asks permission to trust an untrusted local project, shows roster/provider credential status without sending a paid probe, optionally selects provider and model for each role, asks time+tokens for fixed-credit IBM or USD for metered rosters (both time and USD for mixed), accepts editor text or `@file`, then presents an all-effects confirmation before atomically writing only chosen keys. Local writes deliberately overwrite old local debate config to avoid stale model overrides; global writes replace only `settings.json.debate`. Noninteractive setup gives concise command help. The wizard is fake-UI tested, including trust persistence and reloading normalized durations; parser tests cover every documented syntax and invalid inputs. **Constraint made explicit:** “credentials configured” is a live pi credential-resolution status, not a model invocation — a real probe would spend tokens or money merely by opening setup. |
